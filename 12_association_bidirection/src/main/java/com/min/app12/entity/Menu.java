...
/*
 * 단 방향 캑체로 만드는 경우 Menu 와 Category 엔티티
 * Menu                                 Category
 * M                                    1
 ...
 * 
 * 양방향 객체로 만드는 경우 두 연관 관계 중에서 주인(Owner)를 정합니다.
 * 주인(Owner)은 비지니스 로직 상에서 더 중요한 엔티티를 찾는 것이 아닙니다.
 * 주인(Owner)은 단순히 외래키 관리자에게 부여합니다. (외래키를 가지고 있는 엔티티가 주인입니다.)
 * 이 예제의 주인(Owner)은 Menu 엔티티 입니다.
