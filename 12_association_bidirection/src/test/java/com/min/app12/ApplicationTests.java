...
class ApplicationTests {
  ...
  void closeEntityMananger() throws Exception {
	...
	void 양방향_객체_그래프_탐색_select_test() {
	   ...
	   * 
	   * 해결
	   * 주인이 아닌 엔티티의 toString() 메소드에 주인 엔티티의 toString() 메소드 호출 부분을 제거 합니다.
	   * 
	   *   
	      - Menu.java-
	      @Override
	   ...
	   * 
	   * 주의
	   * 롬복 라이브러리의 @toString 을 사용하지 않도록 주의해야 합니다.
	   */
	  ...
	  foundCategory.getMenuList().forEach(System.out::println); // 호출할 메소드만 작성하는 메소드 참조 방식입니다.
	}
	 ...
	 void 양방향_주인_엔티티_insert_test() {
	   ...
	   menu.setCategory(entityManager.find(Category.class, 4));  // 한식 카테고리(catagory_code = 4)에 포함됩니다.
