...
public class User {
  ...
  @GeneratedValue(
      ...
      // strategy = GenerationType.AUTO      // 기본키 생성 방식을 데이터베이스에 따라서 자동으로 선택합니다. (MySQL 의 경우 시퀀스 테이블, Oracle 의 경우 SEQUENCE 등)
      // strategy = GenerationType.SEQUENCE  // 시퀀스 데이터베이스 객체를 이용합니다. (Oracle)
      
      strategy = GenerationType.TABLE        // 키 값을 생성하는 별도의 테이블을 이용합니다.
    , generator = "user_seq_table_generator" // @TableGenerator 를 이용해서 키 값을 생성하는 별도의 테이블을 만듭니다.
  ...
  @Column(name = "user_password", nullable = false)  // 널 값을 가질 수 없습니다. (NOT NULL) 디폴트는 true 입니다.
