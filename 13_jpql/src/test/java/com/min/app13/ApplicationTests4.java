...
class ApplicationTests4 {
  ...
  void 서브쿼리_select_test() {
    ...
    // JPQL 에서는 서브쿼리 SELECT 절과 FROM 절에서는 사용할 수 없습니다.
    // JPQL 에서는 서브쿼리를 조건절(WHERE, HAVING)에서만 사용할 수 있습니다.
    
    // 문제. 카테고리 이름이 "한식"인 메뉴를 조회하기 (카테고리 이름이 "한식"인 카테고리의 카테고리 코드 조회를 서브쿼리로 사용)
    
    StringBuilder jpql = new StringBuilder();
    jpql.append("SELECT m FROM menu4 m");
    jpql.append(" WHERE m.categoryCode = ");
    jpql.append("(SELECT c.categoryCode FROM category4 c WHERE c.categoryName = '한식')");
  ...
  void 동적쿼리_select_test() {
    ...
    // 문제. 메뉴명에 '버터'가 포함되고, 카테고리 코드가 4인 메뉴를 조횧합니다.
    // 단, 메뉴명과 카테고리 코드는 입력될 수도 있고 입력되지 않을 수도 있다고 가정합니다.
    
    String searchMenuName ="버터";
