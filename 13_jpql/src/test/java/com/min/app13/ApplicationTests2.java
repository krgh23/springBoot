...
class ApplicationTests2 {
  ...
  void 엔티티_프로젝션_test() {
    ...
    Menu menu = entityManager.createQuery(jpql, Menu.class).getSingleResult();
    ...
    // orderable_status -> N 으로 수정하기
    // Dirty Checking : 영속 컨텍스트가 관리하는 엔티티를 수정한뒤, 커밋하면 변경 사항이 데이터베이스에 적용됩니다.
  ...
  void new_DTO_프로젝션_test() {
    ...
    // new 를 이용해 생성한 객체는 영속 컨텍스트에 저장되지 않기 때무에 테스트는 실패합니다.
