...
class ApplicationTests3 {
  ...
  void 내부_조인_test() {
    ...
    // 최초로 내부 조인 SQL 문이 실행됩니다. 해당 조인 SQL 문에서는 Menu 엔티티의 정보만 조회만 합니다.
    // Category 엔티티의 정보는 나중에 필요할 때(여기서는 출력할 때) 다시 조회합니다.
  ...
  void 외부_조인_test() {
    ...
    List<Object[]> menuList = entityManager.createQuery(jpql, Object[].class) //Object[].class 없어도 되지만 경고메세지 때문에 기제
                                           .getResultList();
  ...
  void 페치_조인_test() {
    ...
    // 1. 최초로 내부 조인 SQL 문이 실행됩니다. 해당 조인 SQL 문에서는 Menu 엔티티의 정보만 조회만 합니다. (1번 = 쿼리문이 돌아가는 횟수)
    // 2. 각 Menu 엔티티의 Category 엔티티의 정보는 나중에 필요할 때(여기서는 출력할 때) 다시 조회합니다.
    // 3. 각 Menu 엔티티의 Category 엔티티의 정보를 조회할 때 별도의 SQL문이 최대 Menu 엔티티의 갯수만큼 실행됩니다. (n번 = 쿼리문이 돌아가는 횟수)
    
    // String jpql = "SELECT m FROM menu3 m JOIN m.category c"; 일반 내부 조인
    
    // 패치 조인을 사용하면 최초 조인 SQL 문을 실행할 때 Menu 엔티티와 Category 엔티티 정보를 모두 조회합니다.
