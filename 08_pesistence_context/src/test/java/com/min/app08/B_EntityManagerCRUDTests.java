...
class B_EntityManagerCRUDTests {
  ...
  void insert_test() {
    ...
    try {
      ...
      // 커밋 (엔티티 매니저가 관리하는 모든 엔티티가 관계형 데이터베이스에 반영됩니다. 관계형 데이터베이스에 쿼리를 전송하여 엔티티를 추가합니다.)
  ...
  @Test
    ...
    // 엔티티 매니저를 이용해 엔티티 조회 (조회 시 식별자를 이용해서 조회합니다. 없으면 null 이 반환됩니다.)
  ...
  void update_test() {
    ...
    try {
      ...
      // (두번 연속하면 쿼리문이 업데이트가 안됨 ※※※이유: 저장된 엔티티 상태가 변경되면 쿼리문이 생성※※※)
