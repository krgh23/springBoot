...
/**
 * 사용자의 아이디를 전달 받아 해당 사용자의 정보를 UserDetails 타입으로 반환하는 loadUserByUsername() 메소드만
 * 가지고 있는 UserDetailsService 인터페이스를 구현한 클래스입니다.
 * 
 * AuthenticationProvider - 인증 로직에서 UserDetailsService 를 통해 사용자의 세부
 * 정보(UserDetails)를 획득합니다. - 획득한 사용자 세부 정보와 사용자가 입력한 비밀번호의 일치 여부를 판단합니다.
 * (PasswordEncoder 이용) - 비밀번호가 일치하면 AuthenticationFilter 에 이 정보(Authentication
 * 객체)를 전달합니다. 이 정보는 SecurityContext에 저장됩니다. - 비밀번호가 일치하지 않으면 일반적으로 401
 * Unauthorized 를 반환합니다.
 */
