...
public class SecurityConfig {
  ...
  SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    ...
    http.authorizeHttpRequests(auth -> {
      ...
      auth.requestMatchers("/admin/**").hasAnyAuthority(UserRole.ADMIN.getUserRole());  // "/admin"으로 시작하는 주소는 "ADMIN" userRole을 가진 사용자가 접근 할 수 있는 주소입니다.
      auth.anyRequest().authenticated(); // 모든 요청에 대해서 인증을 요구 합니다. 이 설정이 포함되면 로그인 설정을 반드시 별도로 설정해서 처리해야 합니다.
    });
    ...
    // 로그아웃을 관리합니다.
    http.logout(logout -> {
      logout.logoutRequestMatcher(new AntPathRequestMatcher("/auth/logout"));  // "/auth/logout" 요청이 오면 로그아웃합니다.
      logout.invalidateHttpSession(true);  // 로그아웃하면 세션 만료 처리합니다.
      logout.deleteCookies("JSESSIONID");  // 세션 아이디가 저장되어 있는 JSESSIONID 쿠키를 삭제합니다.
