...
class ApplicationTests {
  ...
  void closeEntityMananger() throws Exception {
	...
	void 일대다_객체_그래프_탐색_select_test() {
	  ...
	  // 카테고리 조회 결과 확인
	  // 지연 로딩 사용하면 이 때 메뉴가 조회 됩니다. ( @OneToMany(fetch = FetchType.LAZY) )
	  /*
      select
     ...
     category.setCategoryCode(21);  // 기존 1 ~ 12 은 피해서 신규로 생성합니다
     category.setCategoryName("신규2");
     category.setRefCategoryCode(null);
	   
	   // 신규 카테고리 속하는 신규 메뉴 2개 엔티티 생성
     Menu menu1 = new Menu();
     menu1.setMenuCode(101);  // 기존 1 ~ 21 은 피해서 신규로 생성합니다
     menu1.setMenuName("봄날의빙수");
     menu1.setMenuPrice(30000);
     menu1.setCategoryCode(category.getCategoryCode());
     menu1.setOrderableStatus("Y");
     
     Menu menu2 = new Menu();
     menu2.setMenuCode(102);  // 기존 1 ~ 21 은 피해서 신규로 생성합니다
     menu2.setMenuName("한겨울초밥");
     menu2.setMenuPrice(25000);
     ...
     // 등록한 신규 카테고리 조회 (동작하는 쿼리문 확인)
     Category foundCategory = entityManager.find(Category.class, 21);
     
     // 신규 카테고리 조회 결과 확인 (동작하는 쿼리문 확인)
	   System.out.println(foundCategory);
	 }
