...
class ApplicationTests {
  ...
  void closeEntityMananger() throws Exception {
	...
	void 결과_타입_명시_select_test() {
	  ...
	  String sql = "SELECT menu_code, menu_name, menu_price, category_code, orderable_status FROM tbl_menu"; // 모든 칼럼을 의미하는 * 도 사용가능합니다.(가급적 사용하지 말것 성능이슈)
  ...
  @Test
	  void 결과_타입_없는_select_test() {
	   ...
	   // 결과 타입을 명시 하지 않으면 일부 칼럼도 조회할 수 있습니다. 결과 타입은 Object[] 입니다.
	   
	   String sql = "SELECT menu_code, menu_name FROM tbl_menu";
	   
	   Query query = entityManager.createNativeQuery(sql); // 엔티티 타입으로 결과 타입을 지정할 때만 결과 타입을 명시합니다. 
	                                                        // createNativeQuery(sql, Object[].class) 이런식으로 하면 오류남
  ...
  void 조인_test() {
    ...
    sql.append(" INNER JOIN tbl_menu m");
    sql.append("    ON m.category_code = c.category_code");
  ...
  void 외부_조인_test() {
    ...
    // 문제. 모든 카테고리 정보를 조회하고, 해당 카테고리에 속한 메뉴의 개수를 함께 조회하세요
    // 메뉴가 없는경우 0 으로 조회하세요.
    ...
    List<Object[]> categoryList = entityManager.createNativeQuery(sql.toString())
                                               .getResultList();
